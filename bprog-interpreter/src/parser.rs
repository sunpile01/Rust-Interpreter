...
use crate::types::{Stack, OpBinary, WValue as V};
...
fn infix_op(stack: &mut Stack, tokens: &[&str], operation_type: i64, var_and_fun: &mut HashMap<String, V>) {
    if let Some(next_token) = tokens.get(1) {       // Get the first token after the operation
        ...
        println!("Error: Needs to be a codeblock or valid element after the operation you used!");
    }
}
...
fn handle_code_block_case(stack: &mut Stack, tokens: &[&str], operation_type: i64, var_and_fun: &mut HashMap<String, V>) {
...
fn handle_if_or_loop_case(stack: &mut Stack, tokens: &[&str], operation_type: i64, closing_brace_index: usize, var_and_fun: &mut HashMap<String, V>) {
...
fn handle_valid_element_case(stack: &mut Stack, tokens: &[&str], operation_type: i64, var_and_fun: &mut HashMap<String, V>) {
...
fn handle_if_case(stack: &mut Stack, tokens: &[&str], var_and_fun: &mut HashMap<String, V>) {
    ...
    if let Some(second_token) = tokens.get(2) {          // Get the valid symbol or starting brace {
        if second_token.starts_with("{") {
            ...
            println!("Error: Second element not valid for if statement!");
        }
    }
}
